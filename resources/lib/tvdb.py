...
class TVDB(object):
    ...
    def getIdByZap2it(self, zap2it_id):
        ...
        if CACHE_ON:
            result = cache.cacheFunction(self.getIdByZap2it_NEW, zap2it_id)
        else:
    ...
    def getIdByIMDB(self, imdb_id):
        ...
        if CACHE_ON:
            result = cache.cacheFunction(self.getIdByIMDB_NEW, imdb_id)
        else:
    ...
    def getEpisodeByAirdate(self, tvdbid, airdate):
        ...
        if CACHE_ON:
            result = cache.cacheFunction(self.getEpisodeByAirdate_NEW, tvdbid, airdate)
        else:
    ...
    def getEpisodeByID(self, tvdbid):
        xbmc.log("getIdByIMDB Cache")
        if CACHE_ON:
            result = cache.cacheFunction(self.getEpisodeByID_NEW, tvdbid)
        else:
    ...
    def getIdByShowName(self, showName):
        ...
        if CACHE_ON:
            result = cache.cacheFunction(self.getIdByShowName_NEW, showName)
        else:
    ...
    def getBannerByID(self, tvdbid, type):
        xbmc.log("getIdByZap2it Cache")
        if CACHE_ON:
            result = cache.cacheFunction(self.getBannerByID_NEW, tvdbid, type)
        else:
    ...
    def getIMDBbyShowName(self, showName):
        ...
        if CACHE_ON:
            result = cache.cacheFunction(self.getIMDBbyShowName_NEW, showName)
        else:
